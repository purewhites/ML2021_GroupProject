...
def main():
    ...
    # Data loading code
    train_dataset = BatchDataset(phase='train', transform=transforms.Compose([
...
def train(train_loader, model, criterion, optimizer_conv,scheduler_conv, optimizer_fc, scheduler_fc, epoch, step):
    ...
    for i, (input, target) in enumerate(tqdm_gen):
        model.train()
        ...
        self_logits = torch.zeros(2*batch_size, 196).to(device)
        other_logits= torch.zeros(2*batch_size, 196).to(device)
        self_logits[batch_size:], self_logits[:batch_size] = logit2_self, logit1_self
        other_logits[batch_size:], other_logits[:batch_size] = logit2_other, logit1_other
        # compute loss
        ...
        self_scores = softmax_layer(self_logits)[torch.arange(2*batch_size).to(device).long(), torch.cat([labels1, labels2], dim=0)]
        other_scores = softmax_layer(other_logits)[torch.arange(2*batch_size).to(device).long(), torch.cat([labels1, labels2], dim=0)]
        ...
        if i % args.print_freq == 0:
            tqdm_gen.set_description(('Step: {step}  Epoch: [{0}][{1}/{2}] '
                  'Time {batch_time.val:.3f} ({batch_time.avg:.3f}) '
                  'Loss {loss.val:.4f} ({loss.avg:.4f}) '
                  'Prec@1 {top1.val:.3f} ({top1.avg:.3f})'.format(
                   epoch, i, len(train_loader), batch_time=batch_time, loss=losses, top1=top1, step=step)))
            '''
            tqdm_gen.set_description(('Time: {time}\tStep: {step}\t Epoch: [{0}][{1}/{2}]\t'
                  'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
    ...
    val_dataset = RandomDataset(phase = 'val', transform=transforms.Compose([
        transforms.Resize([512,512]),
